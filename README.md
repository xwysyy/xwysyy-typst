...
The layout components (`duo-slide`, `focus-slide`, `grid-slide`, `stack-slide`, `compare-slide`, `stat-slide`, `figure-slide`, `sidebar-slide`) take typed content items (`visual` / `card` / `takeaway` / `plain`) with declared sizing, measure every block, distribute space fill-first, and export `<xwysyy-slide-layout>` v3 telemetry (allocated frame, natural preferred size, 2-D payload bbox, and paint box per object). `scripts/xwysyy-check` turns one query into numeric layout diagnostics with machine-actionable fixes, and with `--pixels` cross-checks the telemetry against the rendered pixels. They replace hand-written `#v()` spacing when a slide is generated by an agent. For stepwise reveal use their `reveal: true` parameter instead of `#pause`, which cannot appear inside the components (touying panics). See [`docs/LAYOUT.md`](docs/LAYOUT.md).
