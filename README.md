...
- The Universe template includes `scripts/xwysyy-check`, which turns one compile into numeric layout diagnostics with machine-actionable fixes and a pixel cross-check; its agent profile gates AI-generated decks.
...
The layout components (`duo-slide`, `focus-slide`, `grid-slide`, `stack-slide`, `compare-slide`, `stat-slide`, `figure-slide`, `sidebar-slide`) take typed content items (`visual` / `card` / `takeaway` / `plain` / `metric`) with declared sizing, measure every block, distribute space fill-first, and export `<xwysyy-slide-layout>` v4 telemetry (allocated frame, natural preferred size, 2-D payload bbox with a measured/declared source, and paint box + fill per object). Projects created with `typst init` include `scripts/xwysyy-check`; run `scripts/xwysyy-check main.typ --profile agent` to turn the telemetry into numeric diagnostics and cross-check it against rendered pixels. For stepwise reveal use the components' `reveal: true` parameter instead of `#pause`, which cannot appear inside the components (touying panics). See [`docs/LAYOUT.md`](docs/LAYOUT.md).
...
Files outside `template/` are licensed under [MIT](./LICENSE). Files in
`template/` are licensed under [MIT-0](./LICENSE-MIT-0), so projects created
from the template may be modified and redistributed without an attribution or
