...
- Tests (97): unit coverage for every diagnostic, the fail-closed parser, the frame machine, and rule validation; integration compiles of the demo (10 good + 6 bad pages), fit-state regressions (`tests/fixtures/layout-fit.typ`), handout coverage regression (`layout-handout.typ`), pixel true positives (`layout-pixel.typ`), adversarial fake-green regressions from the external review that must fail (`tests/fixtures/adversarial/`: auto-id blank first frame, empty stretch visual), and seventeen panic fixtures that must fail to compile in both outputs (`tests/fixtures/panic/`)
