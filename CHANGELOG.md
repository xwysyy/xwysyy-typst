...
- CI coverage for the declared Typst 0.14.0 minimum, the optional `xwysyy-extras.typ` entrypoint, and the generated Universe package shape
...
- Tests (102): 97 layout tests cover every diagnostic, the fail-closed parser, the frame machine, rule validation, real compilation, pixel true positives, adversarial fake-green cases, and seventeen panic fixtures in both outputs; five release-staging CLI tests cover successful publication, manifest types and paths, README contracts, and failure cleanup
...
- Package metadata now declares Typst 0.14.0 as the tested minimum; the package README uses self-contained commands and immutable `v0.4.0` links, and documents the optional extras subpath explicitly
