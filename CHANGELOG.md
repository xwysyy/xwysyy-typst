...
- Tests (109): 97 layout tests cover every diagnostic, the fail-closed parser, the frame machine, rule validation, real compilation, pixel true positives, adversarial fake-green cases, and seventeen panic fixtures in both outputs; nine release-staging and source-contract tests cover successful publication, reviewer-sensitive manifest metadata, manifest types and paths, README contracts, unsupported package subpaths, and failure cleanup; three visual-render tests cover complete publication, failure rollback, and missing-output validation
...
- Package metadata keeps the MIT license and `computer-science` discipline from 0.3.0 while declaring Typst 0.14.0 as the tested minimum; the package README uses self-contained commands and immutable `v0.4.0` links, and documents the optional extras loader explicitly
...
- Visual regression examples use a fixed date under `visual-ci=true`, and `scripts/render-visuals` stages the complete PNG set before replacing its owned output families; failed compiles preserve the prior set, deleted pages cannot linger, temporary test JSON is cleaned automatically, and CLI assertion failures include stdout and stderr
