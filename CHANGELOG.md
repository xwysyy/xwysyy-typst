...
- Tests (103): 97 layout tests cover every diagnostic, the fail-closed parser, the frame machine, rule validation, real compilation, pixel true positives, adversarial fake-green cases, and seventeen panic fixtures in both outputs; six release-staging CLI tests cover successful publication, manifest types and paths, README contracts, unsupported package subpaths, and failure cleanup
...
- Package metadata now declares Typst 0.14.0 as the tested minimum; the package README uses self-contained commands and immutable `v0.4.0` links, and documents the optional extras loader explicitly
