...
The builder includes the manifest, both license files, English README, thumbnail, the main entrypoint, the lazily loaded extras module, `src/`, `template/`, and `docs/LAYOUT.md`. It copies the three end-user QA tools from the source repository's `scripts/` directory into the staged `template/scripts/` directory, so `typst init` places them in the generated project without requiring duplicate source files. It rejects a dirty source repository, an existing output path, an output path inside this repository, malformed package metadata, missing required files, and lost executable permissions. Extraction and verification happen in a temporary sibling directory, so a failed build leaves no partial output. Its final line reports a tree SHA-256 for review.
