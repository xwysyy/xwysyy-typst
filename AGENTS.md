...
| `src/themes.typ` | `themes` 字典（sky / sunset）+ 顶层色变量（`sea` / `sky` / `skyl` / `skyll` / `paper`）+ `_theme-state` + 颜色宏（`red`/`bred`/`yellow`/`byellow`） |
| `src/elements.typ` | 共享 show-chain `xwysyy-elements`（slide 共用）+ `info` + `textbox` |
| `src/note.typ` | 笔记入口 `xwysyy-note`（A4，主题无关，show 规则独立） |
| `src/slides.typ` | slide 入口 `xwysyy-pre` + 7 种版式（`xwysyy-slide`、`title-slide`、`outline-slide`、`new-section-slide`、`focus-slide`、`image-slide`、`end-slide`）。`outline-slide` 自动过滤 `<touying:hidden>` 标签且 >5 章自动两列。`frozen-counters` 默认冻结 `figure` 和 `math.equation` 计数器 |
...
- **改完必编译**：任何对 `xwysyy.typ` / `src/*.typ` / 示例的修改完成后，跑 `typst compile --root . examples/slides-sky.typ && typst compile --root . examples/slides-sunset.typ && typst compile --root . examples/note.typ` 验证 slide 两个主题 + 笔记都无错误才算完成。
...
- **改色 / 新增主题**：编辑 `src/themes.typ` 顶部 `themes` 字典，新增一个 key 即可。每个主题需包含 `sea`/`sky`/`skyl`/`skyll`/`paper`/`header-fill`/`header-text`/`page-fill` 共 8 个字段。
- **改字号**：`src/slides.typ` 内 `xwysyy-pre` 中 `set text(... size: 5.5mm)`；或示例 deck 顶部 `#set text(size: ...)` 临时覆盖。
